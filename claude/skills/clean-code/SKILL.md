...
function check(u: any, r: any): boolean {
  if (u != null) {
    if (u.role == 'admin') {
      return true;
...
function processOrder(order: Order): void {
  ...
  // charge
  const payment = gateway.charge(order.customer.paymentMethod, total);
  if (!payment.success) {
    throw new PaymentError(payment.error);
  }

  // send confirmation
  sendEmail(order.customer.email, "Order confirmed", `Total: $${total.toFixed(2)}`);
}
```
...
function withdraw(account: Account, amount: number): void {
  if (account == null) throw new Error("Account must not be null");
  if (amount > account.getBalance()) {
    throw new InsufficientFundsError(account, amount);
  }
  account.debit(amount);
}
// Caller: try { withdraw(acct, 100); } catch (e) {
//   if (e instanceof InsufficientFundsError) { ... }
// }
```

> Throw typed `Error` subclasses, not error codes. TypeScript exceptions are all
> unchecked, so a new failure mode in a low-level function never forces signature
> changes up the call chain - callers opt in to handling via `instanceof`.
